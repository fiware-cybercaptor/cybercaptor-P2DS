...
/**
 * A MPC privacy peer with the computation capabilities for the uniquecount protocol
 * 
 * @author Dilip Many
 *
 */
public class UniqueCountPrivacyPeer extends UniqueCountBase {
    private static final Logger logger = LogManager.getLogger(UniqueCountPrivacyPeer.class);
    
	/** vector of protocols (between this privacy peer and the peers) */
	private Vector<UniqueCountProtocolPrivacyPeerToPeer> peerProtocolThreads = null;
	/** vector of protocols (between this privacy peer and other privacy peers) */
	private Vector<UniqueCountProtocolPrivacyPeerToPP> ppToPPProtocolThreads = null;
	/** vector of information objects for the connected peers */
	protected Vector<UniqueCountPeerInfo> peerInfos = null;
	/** vector of information objects for the connected privacy peers */
	protected Vector<UniqueCountPeerInfo> privacyPeerInfos = null;
	...
	private int numberOfHonestPeers;

	/**
	 * creates a new MPC uniquecount privacy peer
	 *
	 ...
	 * @throws Exception
	 */
	public UniqueCountPrivacyPeer(int myPeerIndex, ConnectionManager cm, Stopper stopper) throws Exception {
		super(myPeerIndex, cm, stopper);

		peerInfos = new Vector<UniqueCountPeerInfo>();
		privacyPeerInfos = new Vector<UniqueCountPeerInfo>();
		peerProtocolThreads = new Vector<UniqueCountProtocolPrivacyPeerToPeer>();
		ppToPPProtocolThreads = new Vector<UniqueCountProtocolPrivacyPeerToPP>();
	}
	...
	protected void initializeNewRound() {
		...
		finalResults = null;
		numberOfHonestPeers=numberOfInputPeers;
	...
	private void createProtocolThreadsForPrivacyPeers(List<String> privacyPeerIDs, Map<String, Integer> ppIndexMap) {
		...
		for(String ppId: privacyPeerIDs) {
			...
			UniqueCountProtocolPrivacyPeerToPP pp2pp = new UniqueCountProtocolPrivacyPeerToPP(currentID, this, ppId, otherPPindex, stopper);
			pp2pp.setMyPeerIndex(myAlphaIndex);
			pp2pp.addObserver(this);
			Thread thread = new Thread(pp2pp, "UniqueCount PP-to-PP protocol connected with " + ppId);
			ppToPPProtocolThreads.add(pp2pp);
			privacyPeerInfos.add(currentID, new UniqueCountPeerInfo(ppId, otherPPindex));
			thread.start();
			currentID++;
		}
	}
	...
	private void createProtocolThreadsForInputPeers(List<String> inputPeerIDs) {
		...
		for(String ipId: inputPeerIDs) {
			...
			UniqueCountProtocolPrivacyPeerToPeer pp2p = new UniqueCountProtocolPrivacyPeerToPeer(currentID, this, ipId, currentID, stopper);
			pp2p.addObserver(this);
			Thread thread = new Thread(pp2p, "UniqueCount Peer protocol connected with " + ipId);
			peerProtocolThreads.add(pp2p);
			peerInfos.add(currentID, new UniqueCountPeerInfo(ipId, currentID));
			thread.start();
			currentID++;
		}
	}
	...
	protected void notificationReceived(Observable observable, Object object) throws Exception {
		if (object instanceof UniqueCountMessage) {
			UniqueCountMessage msg = (UniqueCountMessage) object;
			...
			if (msg.isInitialSharesMessage()) {
				...
				UniqueCountPeerInfo peerInfo = getPeerInfoByPeerID(msg.getSenderID());
	 ...
	 */
	protected synchronized UniqueCountPeerInfo getPrivacyPeerInfoByIndex(int privacyPeerNumber) {
	 ...
	 */
	protected synchronized UniqueCountPeerInfo getPeerInfoByIndex(int peerNumber) {
	 ...
	 */
	protected synchronized UniqueCountPeerInfo getPeerInfoByPeerID(String peerID) {
		for (UniqueCountPeerInfo peerInfo : peerInfos) {
	...
	public void startProductComputations() {
		logger.info( "STARTING UniqueCount Protocol round");
		...
		logger.info("thread " + Thread.currentThread().getId() + " started the "+operationIDs.length+" product operations");
	}
	...
	public void startFinalResultReconstruction() {
		// get multiplication results shares and add them up
		ShamirSharing ss = primitives.getMpcShamirSharing();
		long result = 0;
		for(int i = 0; i < operationIDs.length; i++) {
			result = ss.modAdd(result, primitives.getResult(operationIDs[i])[0]);
		}
		long finalResultShare = ss.modSubtract(numberOfItems, result);

		initializeNewOperationSet(1);
		operationIDs = new int[1];
		long[] data = null;
		// create reconstruction operation for final result
		operationIDs[0] = 0;
		data = new long[1];
		data[0] = finalResultShare;
		if(!primitives.reconstruct(operationIDs[0], data)) {
			logger.error("reconstruct operation arguments are invalid: id="+operationIDs[0]+", data="+data[0]);
		}
		logger.info("Started the final result reconstruction step ("+operationIDs.length+" reconstruction operations running...)");
	}
	...
	public void setFinalResult() {
		...
		logger.info("Starts next pp-peer protocol step");
	 ...
	 * starts the products.
	 */
	public synchronized void startProducts() {
		logger.info( "STARTING Unique Count products");

		// get shares of honest peers
		long[][] shares = new long[numberOfHonestPeers][];
		int honestPeerIndex = 0;
		for(int inputPeerIndex = 0; inputPeerIndex < numberOfInputPeers; inputPeerIndex++) {
			if(getPeerInfoByIndex(inputPeerIndex).isVerificationSuccessful()) {
				shares[honestPeerIndex] = getPeerInfoByIndex(inputPeerIndex).getInitialShares();
				honestPeerIndex++;
			}
		}
		...
		for(int item = 0; item < numberOfItems; item++) {
			operationIDs[item] = item;
			data = new long[numberOfHonestPeers];
			for(int peerIndex = 0; peerIndex < numberOfHonestPeers; peerIndex++) {
				data[peerIndex] = shares[peerIndex][item];
			}
			if(!primitives.product(item, data)) {
				Services.printVector("SEVERE: product operation arguments are invalid: id="+item, data, logger);
			}
		}
		logger.info("Started the product step ("+operationIDs.length+" products running...)");
	}

	/**
	 * starts the input verification
	 */
	public void startVerification() {
			logger.info( "STARTING Unique Count verification");

			ShamirSharing ss = primitives.getMpcShamirSharing();
			// create multiplication operation set
			initializeNewOperationSet(numberOfItems*numberOfInputPeers);
			operationIDs = new int[numberOfItems*numberOfInputPeers];
			int operationIndex = 0;
			long[] data = null;
			for(int inputPeerIndex = 0; inputPeerIndex < numberOfInputPeers; inputPeerIndex++) {
				for(int metricIndex = 0; metricIndex < numberOfItems; metricIndex++) {
					operationIDs[operationIndex] = operationIndex;
					data = new long[2];
					data[0] = getPeerInfoByIndex(inputPeerIndex).getInitialShares()[metricIndex];
					data[1] = ss.modSubtract(1, getPeerInfoByIndex(inputPeerIndex).getInitialShares()[metricIndex]);
					if(!primitives.multiply(operationIndex, data)) {
						logger.error("multiplication operation arguments are invalid: id="+operationIndex+"; data="+data[0]+","+data[1]);
					}
					operationIndex++;
				}
			}
			logger.info("Started the verification step ("+operationIDs.length+" multiplications running...)");
	}

	/**
	 * starts the input verification
	 */
	public void startVerificationResultReconstruction() {
		// get verification result shares
		...
		for(int i = 0; i < result.length; i++) {
			// create reconstruction operation for result
			operationIDs[i] = i;
			data = new long[1];
			data[0] = result[i];
		...
		logger.info("Started the verification result reconstruction step ("+operationIDs.length+" reconstruction operations running...)");
	}

	/**
	 * processes the input verification result
	 */
	public void processVerificationResult() {
		int operationIndex = 0;
		int numberOfDisqualifiedPeers = 0;
		for(int inputPeerIndex = 0; inputPeerIndex < numberOfInputPeers; inputPeerIndex++) {
			for(int metricIndex = 0; metricIndex < numberOfItems; metricIndex++) {
				operationIndex = inputPeerIndex*numberOfItems + metricIndex;
				if(primitives.getResult(operationIDs[operationIndex])[0] != 0) {
					getPeerInfoByIndex(inputPeerIndex).setVerificationSuccessful(false);
					numberOfDisqualifiedPeers++;
					logger.info("peer " + getPeerInfoByIndex(inputPeerIndex).getID() + " delivered invalid inputs! (at least metric "+metricIndex+" is invalid)");
					break;
				} else {
					getPeerInfoByIndex(inputPeerIndex).setVerificationSuccessful(true);
				}
			}
		}
		numberOfHonestPeers = numberOfInputPeers - numberOfDisqualifiedPeers;
		logger.info("Verification completed. Number of honest peers: " + numberOfHonestPeers);
	}
	

    /**
     * Have enough honest peers been discovered for a round of computation?
     * If not, no reconstruction should take place.
     * 
     * @return true if enough honest peers are participating in this round
     */
    public boolean enoughHonestPeersDiscovered() {
        return (numberOfHonestPeers == numberOfInputPeers);
    }

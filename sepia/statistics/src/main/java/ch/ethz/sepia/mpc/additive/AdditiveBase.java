 ...
 * privacy peers of the MPC ADDITIVE protocol.
 *
 * @author Dilip Many
 *
 */
public abstract class AdditiveBase extends PrimitivesEnabledPeer {
    private static final Logger logger = LogManager.getLogger(AdditiveBase.class);
	 ...
	 */
	public AdditiveBase(int myPeerIndex, ConnectionManager cm, Stopper stopper) {
	...
	protected synchronized void initProperties() throws Exception {
		...
		if(!skipInputVerification) {
			maxElement = Long.valueOf(properties.getProperty(ConfigFile.PROP_MAXELEMENT, Long.toString(shamirSharesFieldOrder-1)));
		}
		...
		if(!skipInputVerification) {
			logger.info("Maximum value accepted: " + maxElement);
		}

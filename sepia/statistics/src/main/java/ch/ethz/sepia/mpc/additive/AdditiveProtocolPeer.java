...
/**
 * A ADDITIVE protocol between a peer and a privacy peer.
 *
 * @author Dilip Many
 *
 */
public class AdditiveProtocolPeer extends AdditiveProtocol {
    private static final XLogger logger = new XLogger(LoggerFactory.getLogger(AdditiveProtocolPeer.class));

	/** reference to the ADDITIVE peer object that started this protocol instance */
	private AdditivePeer inputPeer;
	 ...
	 */
	public AdditiveProtocolPeer(int threadNumber, AdditivePeer inputPeer, String privacyPeerID, int privacyPeerIndex, Stopper stopper) {
	...
	/**
     * Run the ADDITIVE protocol for the peer
     *
	...
	private synchronized void createInitialSharesMessage() {
		...
		messageToSend = new AdditiveMessage(inputPeer.getMyPeerID(), myPeerIndex);

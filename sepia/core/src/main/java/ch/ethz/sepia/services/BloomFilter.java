...
public class BloomFilter {
    private static final XLogger logger = new XLogger(LoggerFactory.getLogger(BloomFilter.class));
	 ...
	 */
	public static double getCardinality (final double nonZeros, final double m, final double k, final boolean counting) {
		if(counting){
			return nonZeros/k;
		}else{
	 ...
	 */
	public static double getExpectedTrueBits(final double n, final double m, final double k){
	 ...
	 */
	public static double getFalsePositiveRate(final double n, final double m, final double k){
	 ...
	 */
	public static double getIntersectionCardinality(final double t_av, final double tB, final double t_Inter,
			final double k, final double m, final double p, final boolean counting){
		if(t_Inter < 1){
			return 0;
		}else{
...
//						(k*Math.log(1-1/m));
	 ...
	 * @return
	 */
	public static int getIntersectionEstimatedTrueBits(final double t_av, final double tbB, final double cardInt,
			final double k, final double m, final double p){
	 ...
	 */
	public static int getNextPowerOfTwo(final int size){
	 ...
	 */
	public static double [] getParameterEstimate(final int elements, final double fpr){
		final double [] result = new double[3];
		// ideal filter length
		final double ideal = elements*1.44*log2(1.0/fpr);
	...
	public static BloomFilter intersection(final BloomFilter[] BFs, final boolean counting){
	...
	private static double inverseExpectedTruebitsIntersection(final double t_av, final double t_final,
			final double p, final double m, final double k){

		logger.info("inverseExpectedTruebitsIntersection: t_av="+t_av+", t_final: "+t_final+", p="+p+", m="+m+", k="+k);
		if(t_final < 1){
			return 0;
		}
	...
	public static BloomFilter thresholdUnion(final BloomFilter[] BFs, final int T, final boolean learnCount){
	...
	public static BloomFilter union(final BloomFilter[] BFs, final boolean counting){
		BloomFilter result = BFs[0];
		for(int i = 1; i < BFs.length; i++){
	...
	public static BloomFilter weightedIntersection(final BloomFilter[] Keys, final BloomFilter[] Weights, final int Tk, final int Tw, final boolean learnCount){
		final BloomFilter k = thresholdUnion(Keys, Tk, false);
		final BloomFilter w = thresholdUnion(Weights, Tw, true);
		final BloomFilter result = k;
	 ...
	 */
	public BloomFilter(final int hashes ,final int size, final boolean counting){
		final int Range = getNextPowerOfTwo(size);
		bf = new int[Range];
		b_cnt = counting;
		nonZeroCounter = 0;
	 ...
	 * @param counting true if a counting BloomFilter is needed
	 */
	public BloomFilter(final int hashes, final int [] filterarray, final boolean counting){
		final int size = filterarray.length;
	 ...
	 * @param counting true if a counting BloomFilter is needed
	 */
	public BloomFilter(final int hashes, final long [] filterarray, final boolean counting){
		final int size = filterarray.length;
		// make sure filter has power of 2
		final int Range = getNextPowerOfTwo(size);
		bf = new int[Range];
	 ...
	 */
	public boolean check(final byte [] val){
		// assume value is in filter
		boolean found = true;

		// compute hashes
		final int [] indices = mhash.hash(val);
	 ...
	 * @return true if value was found
	 */
	public boolean check(final String val) {
		boolean found = false;
		try{
			// use a specific encoding to guarantee the same outcome on all platforms
			final byte [] msg = val.getBytes("ISO-8859-1");
			found = check(msg);
	 ...
	 */
	public int getCount(final byte[] val){
	...
	public int getCount(final int val){
	...
	public boolean insert(final byte [] val) {
		return insert(val, 1);
	}
	 ...
	 */
	public boolean insert(final byte [] val, final int count) {
		int coll_count = 0;
		// compute hashes
		final int [] indices = mhash.hash(val);
	...
	public boolean insert(final int val, final int count){
		return insert(intToByteArr(val), count);
	}

	public boolean insert(final String val) {
		return insert(val, 1);
	}
	 ...
	 * (either false positive, or value already present)
	 */
	public boolean insert(final String val, final int count) {
	 ...
	 */
	public BloomFilter intersect(final BloomFilter rhs, final boolean cnt){
		final BloomFilter filt = new BloomFilter(this.getHashCount(), this.bf, cnt);
		if(!isComparable(rhs)){
			return null;
		}else{
	 ...
	 */
    private byte[] intToByteArr(final int val) {
	...
	public boolean isComparable(final BloomFilter rhs){
	 ...
	 */
	public BloomFilter join(final BloomFilter rhs, final boolean cnt){
		final BloomFilter filt = new BloomFilter(this.getHashCount(), this.bf, this.isCounting());
		if(!isComparable(rhs)){
			return null;
		}else{
			if(cnt){ // sum
	 ...
	 */
	public BloomFilter reduceBy(final int threshold){
		final BloomFilter filt = new BloomFilter(this.getHashCount(), this.bf, this.isCounting());
	 ...
	 * @return true if remove was successful
	 */
	public boolean remove(final String val) {
	...
	public void writeToFile(final String fileName) throws Exception {

...
public abstract class PrimitivesEnabledProtocol extends ProtocolBase  {
    private static final Logger logger = LogManager.getLogger(PrimitivesEnabledProtocol.class);
...
//	protected PeerInfo otherPeerInfo = null;
	...
	public boolean doOperations() throws PrimitivesException, InterruptedException, BrokenBarrierException, PrivacyViolationException  {
		...
		logger.info("thread " + Thread.currentThread().getId() + " completed " + ids.length + " operations (operationSet=" + currentOperationSetNumber + ")");
		return true;
	}
	...
	private void generateRandomNumbersIfNeeded() throws PrimitivesException, InterruptedException, BrokenBarrierException, PrivacyViolationException {
		...
		if (primitivesEnabledPeer.getBarrierPP2PPProtocolThreads().await()==0) {
			logger.info("thread " + Thread.currentThread().getId() + ": Automatically batch-generating "+randomNumbersNeeded+" bitwise-shared random numbers!");
	...
	private void sendReceiveOperationData(int recipientPrivacyPeerIndex) throws PrimitivesException, PrivacyViolationException {
		...
		logger.info("Thread "+Thread.currentThread().getId()+": Send/receive Shamir Sharing Protocol Primitives message");
	...
	protected synchronized void sendOperationData() throws PrivacyViolationException {
		logger.info("Sending Shamir Sharing Protocol Primitives message (to " + otherPeerID + ")");
	...
	protected synchronized void receiveOperationData() throws PrivacyViolationException {
		logger.info("Waiting for Shamir Sharing Protocol Primitives message to arrive (from " + otherPeerID + ")");
		...
		if (messageType==null || messageReceived==null) {
			...
			logger.warn("Received empty message from "+otherPeerID+". Using a DUMMY message instead.");
		}
		...
		if (messageType.equals(SHAMIR_SHARING_PROTOCOL_PRIMITIVES_MESSAGE)) {
			...
			logger.warn("Received unexpected message type (expected: " + SHAMIR_SHARING_PROTOCOL_PRIMITIVES_MESSAGE + ", received: " + messageType);
		}
	}
    ...
    public static void logStatistics() {
    	...
    	logger.info("--- Total      : Computation: "+totalComputationTime/1000.0+ ", Communication: "+totalCommunicationTime/1000.0);
    	logger.info("--- This round : Computation: "+thisRoundComputationTime/1000.0+ ", Communication: "+thisRoundCommunicationTime/1000.0); 
    	logger.info("--- Avg. round : Computation: "+(totalComputationTime/1000.0)/(numberOfFinishedRounds+1)+",  Communication: "+(totalCommunicationTime/1000.0)/(numberOfFinishedRounds+1)+" ("+100*((double)totalCommunicationTime)/(totalCommunicationTime+totalComputationTime) +"%)");
    }

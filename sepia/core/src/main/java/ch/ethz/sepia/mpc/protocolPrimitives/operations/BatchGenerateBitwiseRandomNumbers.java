...
public class BatchGenerateBitwiseRandomNumbers extends GenericOperationState implements IOperation {
    private static final XLogger logger = new XLogger(LoggerFactory.getLogger(BatchGenerateBitwiseRandomNumbers.class));
	/**
	 ...
	 */
	public BatchGenerateBitwiseRandomNumbers(final long[] data) {
	...
	public void doStep(final Primitives primitives) throws PrimitivesException {
		...
		if(getCurrentStep() == 1) {
			...
			final double nextLargerPowerOfTwo = Math.pow(2, Long.toBinaryString(primitives.getFieldSize()).length());
			final double dFieldSize = (double)primitives.getFieldSize();
			final long bitwiseGenerationAttempts = (long)(getInitialData()[0] / (dFieldSize/nextLargerPowerOfTwo)) +2;
			final long bitGenerationAttempts = (long)(bitwiseGenerationAttempts*primitives.getBitsCount() / ((dFieldSize-2)/dFieldSize) );
		...
		if(getCurrentStep() == 2) {
			...
			if(completedBitGenerationAttempts == getSubOperations().length) {
				...
				final IOperation[] subOperations = new IOperation[nextResult/primitives.getBitsCount()];
				long[] data = null;
				int nextBit = 0;
				for(int i = 0; i < subOperations.length; i++) {
		...
		if(getCurrentStep() == 3) {
			...
			if(completedBitwiseGenerationAttempts == getSubOperations().length) {
				final long[] intermediaryResult = new long[nextResult*primitives.getBitsCount()];
				int nextBit = 0;
				for(int i = 0; i < getSubOperations().length; i++) {
					if(getSubOperations()[i].isOperationCompleted()) {
				...
				if(nextResult < getInitialData()[0]) {
					logger.info("successfully generated "+nextResult+" bitwise shared random numbers, but need "+getInitialData()[0]);
					final IOperation[] subOperations = new IOperation[1];
		...
		if(getCurrentStep() == 4) {
			...
			if(getSubOperations()[0].isOperationCompleted()) {
				// copy results
				final int intermediaryResultLength = getIntermediaryResult().length;
				final long[] finalResult = new long[intermediaryResultLength + getSubOperations()[0].getFinalResult().length];

...
public class PeerFactory {
	...
	public synchronized static PeerBase getPeerInstance(final String peerName,
		...
		final Properties props = Configuration.getInstance(peerName).getProperties();
		if (isInputPeer) {
	 ...
	 */
	private static PeerBase getPeerInstanceByName(final String className,
			final int peerNumber, final ConnectionManager cm,
			final Stopper stopper) throws ClassNotFoundException,
			SecurityException, NoSuchMethodException, IllegalArgumentException,
			InstantiationException, IllegalAccessException,
			InvocationTargetException {
		final Class<?> c = Class.forName(className);
		final Constructor<?> con = c.getConstructor(new Class[] { int.class,
				ConnectionManager.class, Stopper.class });

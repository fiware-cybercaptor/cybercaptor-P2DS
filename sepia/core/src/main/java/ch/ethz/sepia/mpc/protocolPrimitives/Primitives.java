...
public class Primitives {
    ...
    private static final Logger logger = LogManager.getLogger(Primitives.class);
	...
	public Primitives(String randomAlgorithm, long fieldSize, int degreeT, int numberOfPrivacyPeers, int myPrivacyPeerIndex, int protocolThreadsCount) {
		...
		stateVariables = new Stack<Object>();
...
//		operationLogs = new ArrayList<String>(operationsCount);
	...
	public synchronized void initialize(int parallelOperationsCount, int totalOperationsCount) {
		if(parallelOperationsCount >= totalOperationsCount) {
			logger.info("parallelOperationsCount >= totalOperationsCount: "+parallelOperationsCount+" >= "+totalOperationsCount+"; will use initialize("+totalOperationsCount+") instead");
...
//		operationLogs = new ArrayList<String>(parallelOperationsCount);
	...
	public synchronized void setReceivedData(int privacyPeerIndex, long[] data) {
		...
		if (data!=null) {
			logger.info("setReceivedData set " + data.length + " shares for " + runningOperationsCount + " operations");
		} else {
			logger.info("setReceivedData set: DUMMY shares for " + runningOperationsCount + " operations");
		}
	}
		...
		if(!areOperationsCompleted()) {
			...
			if(operationsQueue != null) {
				if(operations.size()+operationsQueue.size() < totalOperationsCount) {
					logger.warn("Not all operations of operation set submitted yet! (Running operations: "+operations.size()+"; queued: "+operationsQueue.size()+"; expected total: "+totalOperationsCount+")");
				}
			} else {
				if(operations.size() < totalOperationsCount) {
					logger.warn("Not all operations of operation set submitted yet! (Running operations: "+operations.size()+"; no queue; expected total: "+totalOperationsCount+")");
				}
			}
...
//					currentOperationID = ids[i];
	...
	public boolean bitwiseLessThan(int id, long[] data) {
		...
		if( (data.length-1) != (2*bitsCount) ) {
			logger.warn("Incorrect amount of arguments: expected="+(1+(2*bitsCount))+", received="+data.length);
			return false;
		}
		if(!( (data[0] == 0) || (data[0] == 1) || (data[0] == 2) )) {
			logger.warn("Incorrect type: must be 0,1, or 2; received="+data[0]);
			return false;
		}
	...
	public boolean generateBitwiseRandomNumber(int id, long[] data) {
		...
		if(data != null) {
			if(data.length != bitsCount) {
				logger.warn("creation of bitwise (shared) random number generation operation takes either none or "+bitsCount+" arguments!");
				return false;
			}
		}
	...
	public boolean batchGenerateBitwiseRandomNumbers(int id, long[] data) {
		...
		if(data != null) {
			...
			if((data[0] < 1) || (data[0]*(long)bitsCount > (long)Integer.MAX_VALUE)) {
				logger.warn("requested amount of bitwise shared random numbers must be between 1 and "+((long)Integer.MAX_VALUE)/bitsCount+"! (received: "+data[0]+")");
				return false;
			}
		}
	...
	public boolean leastSignificantBit(int id, long[] data) {
		...
		if((data.length != 1) && (data.length != (1+bitsCount)) ) {
			logger.warn("creation of least significant bit operation takes either one or "+(1+bitsCount)+" arguments!");
			return false;
		}
	...
	public boolean lessThan(int id, long[] data) {
		...
		if((data.length != 5) && (data.length != (5+bitsCount)) && (data.length != (5+2*bitsCount)) && (data.length != (5+3*bitsCount)) ) {
			logger.warn("creation of less-than operation takes either 5, "+(5+bitsCount)+", "+(5+2*bitsCount)+" or "+(5+3*bitsCount)+" arguments!");
			return false;
		}
